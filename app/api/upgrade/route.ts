...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `Ты — опытный продуктовый наставник. Твоя задача — создать ПЕРЕПИСАННУЮ версию ответа студента, где каждое предложение улучшено, уточнено или дополнено конкретикой.
...
===КАК ИМЕННО УЛУЧШАТЬ===
Представь что ты редактор — берёшь каждое предложение студента и прямо по тексту:
- Расплывчатые фразы → заменяй на конкретные с цифрами и терминами
- Неполные мысли → дописывай логичное продолжение прямо там же
- Правильные мысли → оставляй, но делай формулировки чище и точнее
- Пропущенные важные блоки → добавляй как новый абзац в правильном месте по структуре

ЗАПРЕЩЕНО:
- Копировать абзац студента без изменений и добавлять свой блок снизу
- Оставлять "..." или обрывать мысль
- Писать общие фразы без привязки к этому кейсу

РЕЗУЛЬТАТ: upgradedSolution должен читаться как один цельный переписанный ответ, где видна рука студента но каждая мысль доведена до чёткой и конкретной формулировки.

Отвечай ТОЛЬКО в JSON:
{
  "upgradedSolution": "<полностью переписанный ответ студента, markdown, каждый блок улучшен inline>",
  "changes": [
    {
      "section": "<название раздела/блока>",
      "original": "<точная цитата того что написал студент — 1-2 предложения>",
      "improved": "<как именно эта конкретная фраза переписана и почему>",
      "explanation": "<почему это делает ответ сильнее на интервью>"
    }
  ],
  "keyLessons": [
    "<ключевой урок 1 из ошибок этого ответа — конкретно, без общих слов>",
    "<ключевой урок 2>",
    "<ключевой урок 3>"
  ]
}

Изменений 3-6, уроков ровно 3. В changes.original — цитируй точные слова студента.`;
      ...
      messages: [
        {
          role: 'system',
          content:
            'Ты продуктовый наставник-редактор. Ты берёшь ответ студента и переписываешь его inline — каждое предложение становится чище, конкретнее, сильнее. Ты не добавляешь блоки снизу — ты редактируешь текст изнутри, сохраняя голос и логику студента. Отвечай строго в JSON.',
        },
